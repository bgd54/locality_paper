...
Airfoil, implemented using the OP2 DSL~\cite{op2} is the smallest, well 
understood and thoroughly studied among the applications we explored. It is 
...
edge. The \texttt{res\_calc} loop is called 2000 times during the total 
...
on an unstructured mesh with 2.8 million cells.
...
\caption{Low-level performance metrics of \texttt{res\_calc} - global 
coloring vs hierarchical coloring (2.8 million mesh cells). The last column 
...
with AoS layout. Adding the the GPS renumbering, improves performance 
...
compared to the base-line AoS. Given that the partition based reordering is 
...
is spent in \texttt{res\_calc}, therefore the achieved $1.2\times$ speedup on 
\texttt{res\_calc} causes about $1.1\times$ speedup on the whole application. 
...
bandwidths are (understandably) higher. On the V100, we achieved $1.28\times$ 
...
the total run time in the original code this speedup results in $1.14\times$ 
...
that the calculation does not read the values of the indirect data from the 
previous iteration, it only read the direct data.
...
we believe explains why performance appear to be independent of the block size 
...
colors, as we observed on Airfoil. The increase of colors are from $3$ to $4$. 
As such the synchronization overhead is also smaller. The percentage of stalls 
caused by synchronization increases from $12\%$ to just $15\%$. Of course, with 
high occupancy, the latency caused by synchronization can be better hidden by 
...
As can be seen from Figure \ref{fig:volna_speedup}, the locality exploiting 
...
means a $1.18\times$ speedup on the kernel and $1.05\times$ speedup on the whole 
...
\caption{Low-level performance metrics of \texttt{SpaceDiscretization} - 
hierarchical coloring. The last column details the measured performance of 
the original code.}
...
SOD test case with a mesh of 4 million elements. The top three, most 
...
those with Airfoil ($64$ registers, achieving occupancy or around $40\%$), this 
...
leads to increased stalls from synchronization: from $9\%$ to $20\%$, while
...
bandwidth which results in $1.03\times$ speedup on the whole application 
...
and $24$ in the global coloring versions (for the different reordering), and 
$4$, $50$ and $15$ in the hierarchical coloring versions. The number of thread 
...
of these two reasons, the synchronization overhead ($39\%$ stalls were from 
synchronization on the partitioned mesh) couldn't be hidden: there were no 
warps from other blocks to be scheduled in place of the stalled ones because
there was only one block running on each multiprocessor. The difference 
in achieved occupancy also means that the SoA version with two blocks per
multiprocessor gives better performance. Nevertheless, as shown on Figure 
...
much less memory. Although the original require less synchronization, the 
...
\texttt{IntagrateStressForElems}, therefore the achieved $1.75\times$ speedup on
the kernel gives about $1.28\times$ speedup on the whole application. The
...
we achieved $1.84\times$ speedup for this kernel compared to the
...
results in about $1.29\times$ speedup on the whole application.
...
\caption{ Low-level performance metrics of \texttt{IntegrateStressForElems} - 
hierarchical coloring. The last column details the measured performance of 
the original kernel.}
...
($166$ in SoA layout). Also, it achieve, with a block size of $384$ and 
...
in \texttt{compute\_face\_flux}, therefore the achieved $1.75\times$ kernel 
speedup provides about $1.10\times$ speedup for the whole application. The 
...
$12\%$  & $8\%$\\
  ...
  \caption{
Low-level performance metrics of the \texttt{compute\_face\_flux} kernel - 
hierarchical coloring. The last column details the measured performance of 
the original code.}
...
$1.20\times$ speedup for the whole application.
...
high data reuse and number of thread colors.  We create 1D, 2D and 3D
partitions such that these will have an increasing amount of reuse and number 
of colors. 
...
In \texttt{IntegrateStressForElems}, the achieved bandwidth of with the 
original ordering is $134\,\text{GB/s}$ - it uses a row major order. This is 
similar to what we use for a block shape that is $128$ cells long and only $1$ 
...
number of thread colors for each application, and performance will suffer 
above that. The challenge lies in finding a partitioning algorithm that can 
