...
Table \ref{tab:airfoil_counters_glob} show the effect of reordering on
the global colouring approach in the Airfoil application (\textbf{res\_calc}
kernel).

The SoA layout, as mentioned in Section \ref{aos-to-soa}, improves on the
...
strengthened by GPS renumbering, which places data points that are accessed in
consecutive threads close to each other ($19\%$ reduction in global read
transactions compared to AoS).
...
\begin{table}[Htbp]
  ...
  \resizebox{\columnwidth}{!}{
    ...
    % two tables
  \begin{tabular}{|R{3cm}|c|c|c|c||c|c|c|c|}
    \hline
                       Colouring  & \multicolumn{4}{c||}{Global} & \multicolumn{4}{c|}{Hierarchical} \\
                       \hline
                      Reordering  & \multicolumn{2}{c|}{none} & GPS & partition & \multicolumn{2}{c|}{no} & \multicolumn{2}{c|}{partition}\\
                      \hline
               ...
               \hline
                ...
                Achieved Occupancy&  $0.63$ &  $0.45$ &      $0.45$&            $0.45$&    $0.44$ &   $0.52$ &      $0.59$ &   $0.42$\\
                      \hline
   Global Memory Read Transactions&  $52424\text{k}$ &  $45781\text{k}$ &      $41246\text{k}$&            $66775\text{k}$&    $21142\text{k}$ &   $21192\text{k}$ &      $13964\text{k}$ &   $14406\text{k}$\\
                      \hline
  Global Memory Write Transactions&  $14007\text{k}$ &  $14737\text{k}$ &      $13773\text{k}$&            $20733\text{k}$&    $ 5807\text{k}$ &   $ 57883\text{k}$ &      $ 3397\text{k}$ &   $ 3669\text{k}$\\
                      \hline
  ...
  colouring implementation of the \textbf{res\_calc} kernel. Block size is
  $480$.}
  \label{tab:airfoil_counters_glob}
\end{table}

The partition based reordering is primarily intended for the hierarchical
...
The measurement on Figure \ref{fig:airfoil_bw-vs-bs_hier_large} used the
hierarchical colouring and the shared memory approach, that, coupled with the
fact that it uses more registers ($52$ (SoA) and $48$ (AoS)) compared to global
colouring ($48$ and $40$), leads to larger variations in occupancy and
performance as the block size changes; for example, between block sizes $480$
and $448$ ($44\%$ versus $51\%$).

The key goal of this strategy is to better exploit data reuse by using the
shared memory; the results of which show immediately in the number of global
transactions as well: at block size $480$, there is roughly $60\%$ decrease in
global read and write transactions, leading to three times the performance.

%XXX: discuss difference between small and large problem (too small a problem, low achieved occupancy on some launches)
% Note that on the smaller dataset, the difference between the reordering schemes
% is not that pronounced, because the achieved occupancy is smaller: not enough
% threads to fill the multiprocessor.
% TODO that's what I wanted to say, but the actual occupancy is 40-50% w/ METIS,
% 36/58 NR

% (15KB shared, and 9KB shared)
...
of the stalls were caused by synchronisation, compared to $9\%$.
...
problem when the size of the dataset is so small that it decreases the achieved
occupancy (when there are not enough threads in one kernel launch to use the
available resources on the GPU).

Despite the higher locality, in most cases, the AoS version also uses more
registers (sometimes the compiler can optimise the register count of both
versions to the same number), and this may lead to lower occupancy. Hence it is
not always obvious which is the better choice.
...
%                              Achieved Occupancy&    $0.44$ &   $0.52$ &      $0.59$ &   $0.42$\\
%         Device Memory Read Transactions (total)&    $21142\text{k}$ &   $21192\text{k}$ &      $13964\text{k}$ &   $14406\text{k}$\\
%        Device Memory Write Transactions (total)&    $ 5807\text{k}$ &   $ 57883\text{k}$ &      $ 3397\text{k}$ &   $ 3669\text{k}$\\
%           Issue Stall Reasons (Synchronization)&    $  11\%$ &   $   9\%$ &      $  14\%$ &   $  14\%$\\
%              Issue Stall Reasons (Data Request)&    $  69\%$ &   $  68\%$ &      $  61\%$ &   $  63\%$\\
%                         Number of Block Colours&    $       4$ &   $       5$ &      $       8$ &   $       8$\\
%                                    Reuse Factor&    $       2$ &   $       2$ &      $     3.6$ &   $     3.6$\\
%                        Number of Thread Colours&    $       3$ &   $       3$ &      $       4$ &   $       4$\\
%                                       Bandwidth&    $ 212\,\text{GB/s}$ &   $ 227\,\text{GB/s}$ &      $ 270\,\text{GB/s}$ &   $ 225\,\text{GB/s}$\\
%     \hline
%   \end{tabular}
%   }
%   \caption{Collected performance metrics of the hierarchical colouring
%   implementation of the \textbf{res\_calc} kernel.}
%   \label{tab:airfoil_counters_hier}
% \end{table}
...
In measurements of the Volna application (\textbf{SpaceDiscretization} kernel)
with hierarchical colouring (Figure \ref{fig:volna_bw-vs-bs_hier}) the
...
As can be seen from Figure \ref{fig:volna_speedup}, our implementation is more
than two times faster than the original OP2 version.
  ...
  \begin{tabular}{|r|cc|c|c|}
    \hline
      ...
      \hline
        ...
        Global Memory Read Transactions &   $ 9114\text{k}$ &   $ 9166\text{k}$ &   $ 7493\text{k}$ &   $ 7617\text{k}$                \\
       Global Memory Write Transactions &   $ 2438\text{k}$ &   $ 2512\text{k}$ &   $ 1542\text{k}$ &   $ 1640\text{k}$                \\
...
With LULESH (Figure \ref{fig:lulesh_bw-vs-bs_hier}), the partitioning approach
actually worsened performance.

The \textbf{IntegrateStressForElems} kernel uses a mapping with 8 dimensions
(compared to the 2 or 3 as in the case of the previous ones) that has high
connectivity, so the number of colours is quite high: $8$, $16$ and $24$ in the
...
applications (Table \ref{tab:lulesh_counters_hier}).
...
there was only one block running on each multiprocessor.

In terms of reuse, although the gain (from $2.6$ to $4.8$) is larger compared to
Airfoil, it is still not enough to offset the overhead of synchronisation: in
this case, the original ordering of the mesh is better. The difference in
...
\begin{table}[Htbp]
  ...
  \begin{tabular}{|r|cc|c|c|c|}
    \hline
      Reordering  & no & no & partition & partition & original\\
      Indirect data layout &    AOS      &   SOA     &     AOS      &    SOA & SOA\\
      Block size &           320 & 320 & 320 & 320 & 64\\
      \hline
                               Bandwidth (GB/s) &   $ 124$ &   $ 168$ &   $ 129$ &   $ 147$ & $97$\\
                             Achieved Occupancy &   $0.15$ &   $0.29$ &   $0.15$ &   $0.30$ & $0.24$\\
                      Warp Execution Efficiency &   $  98\%$ &   $  97\%$ &   $ 91\%$ &   $  90\%$ & $100\%$\\
        Global Memory Read Transactions (total) &   $33572\text{k}$ & $35408\text{k}$ &   $22888\text{k}$ &   $25027\text{k}$  & $16072\text{k}$\\
       Global Memory Write Transactions (total) &   $12673\text{k}$ & $12704\text{k}$ &   $ 8047\text{k}$ &   $ 8701\text{k}$  & $32689\text{k}$\\
...
limits with block sizes larger than $288$; it didn't run on the original mesh
with any block size, and only with smaller block sizes on the reordered meshes
(Figure \ref{fig:mini_aero_bw_crash}). The other measurements were carried out
...
Although the synchronisation overhead wasn't that high ($3$ and $6$ thread
...
\begin{table}[Htbp]
  ...
  \begin{tabular}{|r|cc|c|c|}
    \hline
      ...
      \hline
                ...
                Global Memory Read Transactions&    $73561\text{k}$ & $82028\text{k}$ & $53403\text{k}$ & $77091\text{k}$ \\
               Global Memory Write Transactions&    $9153\text{k}$ & $10390\text{k}$ & $6694\text{k}$ & $9008\text{k}$ \\
  ...
  \caption{Collected performance metrics of the hierarchical colouring
  implementation of the \textbf{compute\_face\_flux} kernel.}
...
\ref{sec:lulesh-summary}, the meshes of miniAero and LULESH are generated by the
code itself, therefore we can control the block-shapes explicitly by generating
a partition with the mesh. This allows us to understand the compromise between
high data reuse and few thread colours used more by creating 1D, 2D and 3D
blocks (these will have an increasing amount of reuse and number of colours).

While both kernels operate on 3D Cartesian (hex8) meshes, the
\textbf{IntegrateStressForElems} kernel uses a mapping from cells to their
incident vertices, and the \textbf{compute\_face\_flux} kernel maps from
(internal) faces to cells. In the former case, the blocks are created from the
cells as rectangles.
...
shape specifically taylored so that the handcrafted blocks can cover them
without any gaps.

In \textbf{IntegrateStressForElems}, compared to the $134\,\text{GB/s}$
bandwidth of the original ordering (which is a row major order, similar to ours
when we use a block shape that is $128$ cells long and only $1$ cell thin in the
other dimensions) and the $132\,\text{GB/s}$ of the partitioned mesh (detailed
in Section \ref{sec:analysis-of-lulesh}), we achieved $167\,\text{GB/s}$
...
In \textbf{compute\_face\_flux}, we achieved $167\,\text{GB/s}$ bandwidth,
compared to $101\,\text{GB/s}$ achieved with partitioning.

Of course, using handcrafted blocks are ``cheating'', and can only be done when
the mesh itself is not structured. However, these illustrate clearly that when
...
be tuned along the amount of reuse it aims to achieve.
