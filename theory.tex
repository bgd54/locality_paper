...
Due to the lockstep execution, consecutive threads in a warp read the memory at
...
% considered, due to the fact that the library is designed to be integrated into 
% existing applicaitons, and the complexity
...
read/written by the same set of blocks. This is more important when multiple
blocks access the same group of data points, since any cache inefficiencies will
count more than once. To achieve this, we simply sort the data points by the
number of blocks that write them, and within this, the indices of the blocks
themselves. \textbf{[end Re-word]}
...
% \ref{shared-memory-approach} -- the computational kernel needs to be surrounded
% by code provided by our library. Data reorganization and execution planning has
% a one-off cost, and needs to be performed during initialization---our library
...
% kernels. The impact of these transformations need to be considered as a whole on
% the applications, considering re-organization between different computational
% kernels is usually prohibitively expensive.
