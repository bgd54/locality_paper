...
% Note that we can put data that is going to be read or written into shared memory. The partitioning is done with regard to the shared
...
of having to load the start and end index for each block. We found this overhead
...
optimisations. First, the existing data layouts may need to be changed to
accommodate AoS, SoA or reordering optimisations, and any auxiliary datasets
...
by code provided by our library. Data reorganisation and execution planning has
a one-off cost, and needs to be performed during initialisation---our library
...
the applications, considering re-organisation between different computational
